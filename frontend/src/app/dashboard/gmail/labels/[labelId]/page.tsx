...
export default function LabelDetailPage() {
  ...
  const { messages, loading, error, fetchMessages, toggleStar, trashMessage, batchTrash, markRead } = useMessages();
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", width: "100%", minWidth: 0 }}>
      <GmailPageHeader title={`Label: ${labelId}`} onBack={() => router.push("/dashboard/gmail/labels")} />
      <MessageList
        messages={messages}
        loading={loading}
        error={error}
