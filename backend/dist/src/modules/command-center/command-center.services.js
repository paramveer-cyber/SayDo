...
const getHighPriorityLabelId = async (tenantCorsair) => {
    const labels = await tenantCorsair.gmail.db.labels.search({
        data: { name: { contains: HIGH_PRIORITY_LABEL_NAME } },
        limit: 1,
        offset: 0,
    });
    const label = labels[0]?.data;
    return label?.id ?? null;
};
export const getEmailsNeedingAttention = async (tenantCorsair, limit) => {
...
export const getMeetingsToday = async (tenantCorsair, dayStartIso, dayEndIso) => {
...
export const detectScheduleConflicts = (meetingsToday) => {
    ...
    for (let i = 0; i < timedMeetings.length; i++) {
        for (let j = i + 1; j < timedMeetings.length; j++) {
            ...
            if (laterMeeting.startMs >= earlierMeeting.endMs)
                break;
            conflicts.push({
                ...
                overlapStart: new Date(Math.max(earlierMeeting.startMs, laterMeeting.startMs)).toISOString(),
                overlapEnd: new Date(Math.min(earlierMeeting.endMs, laterMeeting.endMs)).toISOString(),
            });
        }
    }
    return conflicts;
};
export const getFollowUpsDue = async (tenantCorsair, limit) => {
    ...
    for (const sentMessage of candidateSentMessages) {
        ...
        if (!hasReplyAfterSending) {
            followUpsDue.push({
                ...
                daysSinceSent: Math.floor((now - sentAtMs) / (24 * 60 * 60 * 1000)),
            });
        }
        if (followUpsDue.length >= limit)
            break;
    }
    return followUpsDue;
};
//# sourceMappingURL=command-center.services.js.map