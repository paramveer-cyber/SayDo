import { test, assert, assertStatus, json, authHeader, summary, BASE_URL, } from "./runner.js";
const COMMAND_CENTER = `${BASE_URL}/command-center`;
const TOKEN = process.env.TEST_TOKEN;
if (!TOKEN) {
    console.error("Set TEST_TOKEN env var to a valid access token with Gmail and Calendar connected.");
    process.exit(1);
}
console.log("\n── Command Center ──");
await test("GET /command-center/overview — returns full overview shape", async () => {
    ...
    assert(Array.isArray(body.emailsNeedingAttention), "emailsNeedingAttention must be an array");
    assert(Array.isArray(body.meetingsToday), "meetingsToday must be an array");
    assert(Array.isArray(body.conflictsDetected), "conflictsDetected must be an array");
...
await test("GET /command-center/overview — respects custom timeMin and timeMax", async () => {
    ...
    const response = await fetch(`${COMMAND_CENTER}/overview?timeMin=${dayStart.toISOString()}&timeMax=${dayEnd.toISOString()}`, { headers: authHeader(TOKEN) });
    await assertStatus(response, 200);
    const body = await json(response);
    assert(Array.isArray(body.meetingsToday), "meetingsToday must be an array");
});
await test("GET /command-center/overview — emailsNeedingAttention items have expected fields", async () => {
    ...
    const first = body.emailsNeedingAttention[0];
    ...
    assert(typeof first.isHighPriority === "boolean", "isHighPriority must be a boolean");
});
await test("GET /command-center/overview — conflictsDetected items have overlap window when present", async () => {
    ...
    const first = body.conflictsDetected[0];
    assert(typeof first.overlapStart === "string", "overlapStart must be a string");
...
//# sourceMappingURL=command-center.test.js.map