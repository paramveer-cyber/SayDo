...
await test("GET /calendar/events — filters by timeMin and timeMax", async () => {
  ...
  const response = await fetch(
    `${CALENDAR}/events?timeMin=${encodeURIComponent(timeMin)}&timeMax=${encodeURIComponent(timeMax)}&singleEvents=true&orderBy=startTime`,
    { headers: authHeader(TOKEN) },
  );
  await assertStatus(response, 200);
});
...
await test("POST /calendar/events — creates an event", async () => {
  ...
  if (body.id) createdEventId = body.id;
...
await test("POST /calendar/events — rejects invalid body (missing required fields when strict)", async () => {
  ...
  assert(
    ...
    "schema is lenient — all fields optional, still should not 500",
  );
