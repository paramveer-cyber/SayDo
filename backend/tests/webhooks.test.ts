...
const makePubSubBody = (data: object) => ({
  message: {
...
await test("POST /webhooks — accepts valid pubsub payload with tenantId", async () => {
  const response = await fetch(`${WEBHOOKS}?tenant=test-tenant-id`, {
    ...
    body: JSON.stringify(
      makePubSubBody({ historyId: "12345", emailAddress: "test@example.com" }),
    ),
  });
  assert(
    response.status === 200 || response.status === 404,
    `expected 200 or 404 (no handler matched), got ${response.status}`,
  );
});

await test("POST /webhooks — handles duplicate historyId gracefully", async () => {
  const payload = JSON.stringify(
    makePubSubBody({ historyId: "duplicate-id", emailAddress: "test@example.com" }),
  );
  ...
  const body = await json<{ success: boolean }>(secondResponse);
