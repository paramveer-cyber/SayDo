...
await test("GET /settings — fetches settings, creating defaults if missing", async () => {
  ...
  const body = await json<{ data: { settings: Record<string, unknown> } }>(
    response,
  );
...
await test("PATCH /settings — updates preferredModel", async () => {
  ...
  await assertStatus(response, 200);
  const body = await json<{ data: { settings: { preferredModel: string } } }>(
    response,
  );
  assert(
    body.data.settings.preferredModel === "gemini-2.5-flash",
    "preferredModel must be updated",
  );
});
...
await test("PATCH /settings — updates approvalsRequired boolean", async () => {
  ...
  const body = await json<{
    data: { settings: { approvalsRequired: boolean } };
  }>(response);
  assert(
    body.data.settings.approvalsRequired === true,
    "approvalsRequired must be true",
  );
});
...
await test("PATCH /settings — accepts keybinds map", async () => {
  ...
  await assertStatus(response, 200);
  const body = await json<{
    data: { settings: { keybinds: Record<string, { key: string }> } };
  }>(response);
  assert(
    body.data.settings.keybinds.archiveEmail?.key === "e",
    "keybind must be persisted",
  );
});
...
await test("PATCH /settings — rejects too many keybinds", async () => {
  const tooManyKeybinds: Record<string, { key: string }> = {};
...
await test("PATCH /settings — accepts null geminiApiKey to clear it", async () => {
  ...
  await assertStatus(response, 200);
  const body = await json<{
    data: { settings: { geminiApiKey: string | null } };
  }>(response);
