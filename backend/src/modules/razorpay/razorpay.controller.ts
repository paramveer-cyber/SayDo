...
const validPaidRoles: PaidRole[] = ["bronze_subscriber", "silver_subscriber", "gold_subscriber"];
...
export async function verifyPayment(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!target_role || !validPaidRoles.includes(target_role as PaidRole)) {
    ...
    const updatedUser = await setUserRole(userId, target_role as PaidRole);
