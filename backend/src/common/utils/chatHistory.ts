...
export function sanitizeHistory(rawHistory: string[] | undefined): string[] {
...
const HISTORY_TURN_LIMITS: Record<UserRole, number> = {
  user: 4,
  bronze_subscriber: 8,
...
export function filterHistoryByPlan(
  history: string[],
  role: UserRole = "user",
): string[] {
...
export function buildMessagesFromHistory(
  ...
  const historyMessages: ModelMessage[] = history.map((entry, index) => ({
